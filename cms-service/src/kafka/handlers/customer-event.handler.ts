...
export class CustomerEventHandler extends BaseEventHandler {
  constructor(
  ...
  @EventPattern(KAFKA_TOPICS.CUSTOMERS)
  async handleCustomerChanged(@Payload() message: any): Promise<void> {
    await this.processEvent(
      message,
      CDC_TABLE_NAMES.CUSTOMERS,
      (event) => this.syncService.syncCustomerEvent(event),
    );
  }
}
