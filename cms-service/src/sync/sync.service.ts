...
export class SyncService {
	...
	async syncCustomerEvent(event: CustomerChangedEvent): Promise<void> {
		...
		if (await this.isEventProcessed(event.eventId)) {
			this.logger.debug(
				`Customer event ${event.eventId} already processed, skipping`
			);
			return;
		}
		...
		try {
			switch (event.type) {
				...
				case EVENT_TYPE.DELETED:
					await this.cmsCustomerRepository.delete({
						coreCustomerId: customerId,
					});
					break;
				...
				default:
					this.logger.warn(
						`Unknown event type: ${event.type} for customer event ${event.eventId}`
					);
					return;
			}
			...
			this.logger.log(
				`Successfully synced customer event: ${event.eventId}`
			);
		} catch (error) {
			this.logger.error(
				`Failed to sync customer event ${event.eventId}`,
				error.stack
			);
			throw error;
		}
	...
	async syncProductEvent(event: ProductChangedEvent): Promise<void> {
		...
		if (await this.isEventProcessed(event.eventId)) {
			this.logger.debug(
				`Product event ${event.eventId} already processed, skipping`
			);
			return;
		}
		...
		try {
			switch (event.type) {
				...
				case EVENT_TYPE.DELETED:
					await this.cmsProductRepository.delete({
						coreProductId: productId,
					});
					break;
				...
				default:
					this.logger.warn(
						`Unknown event type: ${event.type} for product event ${event.eventId}`
					);
					return;
			}
			...
			this.logger.log(
				`Successfully synced product event: ${event.eventId}`
			);
		} catch (error) {
			this.logger.error(
				`Failed to sync product event ${event.eventId}`,
				error.stack
			);
			throw error;
		}
	...
	async syncOrderEvent(event: OrderChangedEvent): Promise<void> {
		...
		if (await this.isEventProcessed(event.eventId)) {
			this.logger.debug(
				`Order event ${event.eventId} already processed, skipping`
			);
			return;
		}
		...
		try {
			switch (event.type) {
				...
				case EVENT_TYPE.DELETED:
					await this.cmsOrderRepository.delete({
						coreOrderId: orderId,
					});
					break;
				...
				default:
					this.logger.warn(
						`Unknown event type: ${event.type} for order event ${event.eventId}`
					);
					return;
			}
			...
			this.logger.log(
				`Successfully synced order event: ${event.eventId}`
			);
		} catch (error) {
			this.logger.error(
				`Failed to sync order event ${event.eventId}`,
				error.stack
			);
			throw error;
		}
